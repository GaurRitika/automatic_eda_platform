...
st.set_page_config(page_title="AUTO-EDA Profiler", page_icon="📊", layout="wide")

# Custom CSS injected for better UI
st.markdown("""
<style>
    /* Metric Card Styling */
...
if uploaded_file:
    ...
    if page == "Overview":
        ...
        with tab1:
            ...
            c1.metric("Rows", stats.get("Number of rows", 0))
            c2.metric("Variables", stats.get("Number of variables", 0))
            c3.metric("Duplicate Rows", stats.get("Duplicate rows", 0))
            c4.metric("Total Size in Memory", stats.get("Total size in memory", "0"))
            
            st.markdown("<br>", unsafe_allow_html=True)
            c5, c6, c7, c8 = st.columns(4)
            c5.metric("Missing Cells", stats.get("Missing cells", 0))
            c6.metric("Missing Cells (%)", stats.get("Missing cells (%)", "0.0%"))
            c7.metric("Numeric Variables", len(df.select_dtypes(include='number').columns))
            c8.metric("Categorical Variables", len(df.select_dtypes(include='object').columns))
        ...
        # -------- TAB 4 --------
        with tab4:
            st.link_button("GitHub Repository", "https://github.com/GaurRitika/automatic_eda_platform")
            st.link_button("Twitter", "https://x.com/Gaur2005Ritika")
    ...
    <div style='text-align: center; margin-top: 50px;'>
        <h1 style='font-size: 3.5rem; background: -webkit-linear-gradient(#4facfe, #00f2fe); -webkit-background-clip: text; -webkit-text-fill-color: transparent;'>🚀 AUTO-EDA Profiler</h1>
        <p style='font-size:1.2rem; color: #666;'>Your intelligent, automated Exploratory Data Analysis sidekick.</p>
        <hr style='width: 30%; margin: 25px auto; border-top: 2px solid #ddd;'>
        <p style='font-size: 1.05rem;'>Drop your heavy <b>CSV</b> file in the sidebar and let the magic reveal <b>Distributions</b>, <b>Correlations</b>, <b>Anomalies</b>, and more instantly!</p>
        <br>
        <br>
        <p style='font-size: 0.9rem; color: #888;'>Made with ❤️ by <b>Ritika Gaur</b> | Inspired by ydata-profiling</p>
    </div>
    """, unsafe_allow_html=True)
